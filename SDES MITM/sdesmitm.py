...
def mitm(plaintext1, ciphertext1, plaintext2, ciphertext2):
    e = []
    d = []   
    kp = [] 
    
    for i in range(0, 512):    
        e.append(encrypt(plaintext1, i))      
        d.append(decrypt(ciphertext1, i))       
    ...
    for i in range(len(k1)):
        e1 = encrypt(plaintext2, k1[i])
        e2 = encrypt(e1, k2[i])
        if e2 == ciphertext2:
            kp2.append((k1[i], k2[i]))
    return kp2


# main
plaintext1, ciphertext1, plaintext2, ciphertext2 = input().split()
result = mitm(int(plaintext1), int(ciphertext1), int(plaintext2), int(ciphertext2))
for i in range(len(result)):
    print("Key1: {0}, Key2: {1}".format(result[i][0], result[i][1]))
